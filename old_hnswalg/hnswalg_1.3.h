...
// #define MULTICAND 8   // default value
// #define COUNTER
    ...
    searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        while (!candidate_set.empty()) {
            ...
            */
            ...
            std::vector<std::vector<std::pair<dist_t, int>>> global_cand(multicand);
            std::vector<std::vector<std::pair<dist_t, int>>> global_top(multicand);

            #pragma omp taskloop collapse(2) shared(neighbors, neighbor_size, visited_array, global_cand, global_top) num_tasks(4)
            for (size_t j = 0; j < neighbors.size(); j++) {
                ...
                // std::cout << "[1]: " << global_cand[0].size() + global_cand[1].size() << std::endl;


                // ftime = omp_get_wtime();
                // timevec[3][threadid] += (ftime - itime) * 1e6;
            }
            ...
            // std::cout << "[2]: " << global_cand[0].size() + global_cand[1].size() << std::endl;

            for (auto local_cand : global_cand) {
                // std::cout << "[3]: " << global_cand[0].size() + global_cand[1].size() << std::endl;
            ...
            // std::cout << "[4]: " << candidate_set.size() << std::endl;
